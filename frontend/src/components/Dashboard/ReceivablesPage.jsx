...
const ReceivablesPage = ({ onBack }) => {
  ...
  const shortcuts = [
    { name: 'Sales Invoice', count: 0, form: 'sales-invoice' },
    { name: 'POS Invoice', count: 0, form: 'pos-invoice' },
    { name: 'Payment Entry', count: 0, form: 'payment-entry' },
    { name: 'Journal Entry', count: 0, form: 'journal-entry' },
    { name: 'Accounts Receivable', count: 0, form: 'accounts-receivable' },
    { name: 'Cost Center', count: 0, form: 'cost-center' }
  ];
  ...
  if (currentForm === 'pos-invoice') {
    return <POSInvoiceForm onBack={handleBackToReceivables} />;
  }
  if (currentForm === 'payment-entry') {
    return <PaymentEntryForm onBack={handleBackToReceivables} />;
  }
    ...
    <div className="p-6 space-y-6 bg-gray-50 min-h-full">
      ...
      <div className="mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          {/* Dunning */}
          <div className="bg-white rounded-lg border border-gray-200 p-4">
            <h3 className="font-semibold text-gray-900 mb-3">Dunning</h3>
            <div className="space-y-2">
              {dunningItems.map((item, index) => (
                <div key={index} className="flex items-center justify-between py-2 hover:bg-gray-50 rounded cursor-pointer">
                  <span className="text-sm text-gray-700">{item.name}</span>
                  <ArrowUpRight className="w-4 h-4 text-gray-400" />
                </div>
