...
const NewSalesInvoiceForm = ({ onBack, onSave }) => {
  ...
  const handleTimeSheetChange = (index, field, value) => {
    setTimeSheets(prev => prev.map((item, i) => 
      i === index ? { ...item, [field]: value } : item
    ));
  };
  ...
  const totalTimeSheetAmount = timeSheets.reduce((sum, sheet) => sum + (parseFloat(sheet.billingAmount) || 0), 0);
  
  const grandTotal = totalAmount + totalTaxes + totalTimeSheetAmount;
  ...
  const outstandingAmount = roundedTotal; // Assuming no advance payment
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <div>
          ...
          >
            <option value="SINV-.YY.-">SINV-.YY.-</option>
            <option value="SINV-.MM.-">SINV-.MM.-</option>
          </select>
        ...
        <div className="flex items-center space-x-4 pt-8">
          <label className="flex items-center">
            <input
              type="checkbox"
              checked={formData.isRateAdjustment}
              onChange={(e) => handleInputChange('isRateAdjustment', e.target.checked)}
              className="rounded border-gray-300 text-blue-600 focus:ring-blue-500"
            />
            <span className="ml-2 text-sm text-gray-700">Is Rate Adjustment Entry (Debit Note)</span>
          </label>
        </div>
      </div>
      ...
      <div>
        ...
        <h4 className="text-md font-medium text-gray-900 mb-4">Sales Taxes and Charges</h4>
        ...
        <div className="flex justify-end">
          <div className="w-80 space-y-4">
            <div className="flex justify-between">
              <span className="text-lg font-medium text-gray-900">Grand Total (INR) <span className="text-red-500">*</span></span>
            ...
            <div className="flex items-center justify-between">
              <label className="flex items-center">
                <input
                  ...
                  className="rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                />
                <span className="ml-2 text-xs text-gray-600">Use Company default Cost Center for Round off</span>
              </label>
            </div>
            ...
            <div className="flex justify-between">
              <span className="text-sm text-gray-600">Total Advance (INR)</span>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <label className="flex items-center">
                  <input
                    type="checkbox"
                    checked={formData.isCashDiscount}
                    onChange={(e) => handleInputChange('isCashDiscount', e.target.checked)}
                    className="rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                  />
                  <span className="ml-2 text-sm text-gray-700">Is Cash or Non Trade Discount</span>
                </label>
              </div>
              <div>
      ...
      <div>
        <button
          onClick={() => toggleSection('timeSheetList')}
          className="flex items-center justify-between w-full text-left mb-4"
        >
          <h2 className="text-lg font-medium text-gray-900">Time Sheet List</h2>
          <ChevronDown className={`w-5 h-5 text-gray-400 transform transition-transform ${
            expandedSections.timeSheetList ? 'rotate-180' : ''
          }`} />
        </button>
        
        {expandedSections.timeSheetList && (
          <div>
            <h4 className="text-md font-medium text-gray-900 mb-4">Time Sheets</h4>
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Billing Hours</th>
                      <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Billing Amount</th>
                      <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Actions</th>
                    </tr>
                  </thead>
                  <tbody className="bg-white divide-y divide-gray-200">
                    {timeSheets.map((sheet, index) => (
                      <tr key={index}>
                        ...
                        <td className="px-4 py-3 text-sm text-gray-900">{sheet.id}</td>
                        <td className="px-4 py-3">
                          <input
                            type="text"
                            value={sheet.activityType}
                            onChange={(e) => handleTimeSheetChange(index, 'activityType', e.target.value)}
                            className="w-full px-2 py-1 border border-gray-300 rounded text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                        ...
                        <td className="px-4 py-3">
                          <input
                            ...
                            onChange={(e) => handleTimeSheetChange(index, 'description', e.target.value)}
                            className="w-full px-2 py-1 border border-gray-300 rounded text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                        ...
                        <td className="px-4 py-3">
                          <input
                            type="number"
                            value={sheet.billingHours}
                            onChange={(e) => handleTimeSheetChange(index, 'billingHours', e.target.value)}
                            className="w-full px-2 py-1 border border-gray-300 rounded text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                            placeholder="0"
                          />
                        </td>
                        <td className="px-4 py-3">
                          <input
                            type="number"
                            value={sheet.billingAmount}
                            onChange={(e) => handleTimeSheetChange(index, 'billingAmount', e.target.value)}
                            className="w-full px-2 py-1 border border-gray-300 rounded text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                            placeholder="0.00"
                          />
                        </td>
          ...
          <div className="flex items-center justify-between h-16">
            ...
            <div className="flex items-center space-x-4">
              ...
              <nav className="flex items-center space-x-2 text-sm">
                ...
                <ChevronRight className="w-4 h-4 text-gray-400" />
                <span className="text-gray-900">New Sales Invoice</span>
              </nav>
            </div>
...
const SalesInvoiceForm = () => {
  ...
  const handleSaveSalesInvoice = async (invoiceData) => {
    try {
      const response = await fetch('http://localhost:5000/api/sales-invoices', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        console.error('Failed to save sales invoice');
      }
    } catch (error) {
  ...
  if (showNewForm) {
    return <NewSalesInvoiceForm onBack={() => setShowNewForm(false)} onSave={handleSaveSalesInvoice} />;
  }
          ...
          <div className="p-6 border-b border-gray-200">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-3">
                ...
                <h2 className="text-lg font-semibold text-gray-900">Sales Invoice</h2>
              </div>
              <div className="flex items-center space-x-4">
                <div className="relative">
                  <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 w-4 h-4 text-gray-400" />
                ...
                {filteredInvoices.length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-12">
                    ...
                    <h3 className="text-lg font-medium text-gray-900 mb-2">
                      You haven't created any Sales Invoice entries yet
                    </h3>
                    ...
                    <button 
                      ...
                      className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700"
                    >
                      Create your first Sales Invoice entry
                    </button>
                  </div>
                ) : (
