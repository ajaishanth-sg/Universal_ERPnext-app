...
from models.maintenance_scheduling import MaintenanceScheduling
from typing import Dict, Any

router = APIRouter()
collection = db.maintenance_scheduling
...
async def get_maintenance_schedules():
    ...
    async for item in collection.find():
        ...
        item["id"] = item["_id"]
        items.append(MaintenanceScheduling(**item))
    return items

@router.post("/")
async def create_maintenance_schedule(item: MaintenanceScheduling):
...
async def get_maintenance_schedule(item_id: str):
    ...
    item["_id"] = str(item["_id"])
    item["id"] = item["_id"]
    return MaintenanceScheduling(**item)
...
async def update_maintenance_schedule(item_id: str, updates: Dict[str, Any]):
    ...
    return MaintenanceScheduling(**updated_item)
